...
Autom::Autom():
    ...
    adv_in_front(true)
   {
       Serial.println("[SLAVE][init]");
        reset_tics_odos();
        send_cmd();
        moteur_droit.attach(PIN_SLA_MOT_R ); //PIN_PWM_MOTEUR_PROPU_DROITE
        moteur_gauche.attach(PIN_SLA_MOT_L); //PIN_PWM_MOTEUR_PROPU_GAUCHE
        stop();
        bavardeur_sonar_off();
   }
...
void Autom::run(){
    ...
    if(period_sonar.is_over())
    {
        ...
        if(bavardeur_sonar)
        {
            ...
            */
            Serial.println("");
            adv_in_front = get_control()->get_sonar_g()->adv_detected(real_coord);
            Serial.print("[ADV on MAP] ->" );
            Serial.println(adv_in_front);
