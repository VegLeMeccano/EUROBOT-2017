...
void Plateau_jeu::display()
{
    ...
    for(int i = 0; i<5; i++)
    {
        cout << "[PRISE] Module Centraux #" << i << " [" << module_centraux[i]->is_present() << "] (" << module_centraux[i]->get_coord()->get_x() << " ; " << module_centraux[i]->get_coord()->get_y() << ")"<< endl ;
    }

    cout << "[PRISE] Distributeur Monochrome " <<  " [" << distributeur_monochrome->nb_module_available() << "] (" << distributeur_monochrome->get_coord()->get_x() << " ; " << distributeur_monochrome->get_coord()->get_y() << ")"<< endl ;
    cout << "[PRISE] Distributeur Polychrome " <<  " [" << distributeur_polychrome->nb_module_available() << "] (" << distributeur_polychrome->get_coord()->get_x() << " ; " << distributeur_polychrome->get_coord()->get_y() << ")"<< endl ;
...
{
    return depose_module_zone_depart->point_marked() + depose_module_base_laterale->point_marked() + depose_module_base_centrale_diagonale->point_marked()+ depose_module_base_centrale_verticale->point_marked();
}
